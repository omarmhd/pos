...
<style>
    * { margin:0; padding:0; box-sizing:border-box; }
    ...
    .amount-box .value { font-size: 22px; font-weight: bold; color: #7b1f1f; }
    .amount-box .currency { font-size: 12px; }

    .je-table { width: 100%; border-collapse: collapse; margin-top: 8px; font-size: 11px; }
    .je-table th { background: #7b1f1f; color: #fff; padding: 4px 6px; text-align: right; }
...
<div class="page">
    ...
    <div class="voucher-title-bar">
        <span class="title">سند صرف &nbsp;PAYMENT VOUCHER</span>
        <span class="number">{{ $payment->voucher_number }}</span>
    </div>

    <table class="info-table">
        <tr>
            <td class="label">التاريخ</td>
            <td>{{ $payment->voucher_date->format('Y/m/d') }}</td>
            <td class="label">طريقة الصرف</td>
            <td>{{ $payment->paymentMethodLabel() }}</td>
        </tr>
        <tr>
            <td class="label">صُرف إلى</td>
            <td colspan="3"><strong>{{ $payment->paid_to }}</strong></td>
        </tr>
    ...
    <div class="signatures">
        ...
        <div class="sig-box">المُستلِم<br>Received by</div>
    </div>

    <div class="footer">
        {{ $storeName }} — سند صرف رقم {{ $payment->voucher_number }}
